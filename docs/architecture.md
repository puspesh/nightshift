...
Lock files live at `~/.nightshift/<repo>/locks/ns-dev-<role>.lock` and
contain JSON:
...
```
+-------------------------------------------+
|  Agent Profiles (~/.claude/agents/)       |  <- Managed by nightshift
|  Pipeline machinery, state machine,       |     Same across all repos
