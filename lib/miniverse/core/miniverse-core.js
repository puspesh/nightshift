// nightshift: smooth rendering toggle (issue #23) — set to !1 to revert to pixelated
const SMOOTH_RENDERING = !0;
class G {
  constructor() {
...
class K {
  constructor(e, t, i, n) {
    this.layers = [], this.animationId = null, this.lastTime = 0, this.scale = n, this.canvas = document.createElement("canvas"), this.canvas.width = t, this.canvas.height = i, this.canvas.style.imageRendering = SMOOTH_RENDERING ? "auto" : "pixelated", this.canvas.style.width = `${t * n}px`, this.canvas.style.height = `${i * n}px`; // nightshift: issue #23
    const s = this.canvas.getContext("2d");
    if (!s) throw new Error("Could not get 2D context");
    this.ctx = s, this.ctx.imageSmoothingEnabled = SMOOTH_RENDERING, SMOOTH_RENDERING && (this.ctx.imageSmoothingQuality = "high"), this.camera = new G(), e.appendChild(this.canvas); // nightshift: issue #23
  }
  addLayer(e) {
  ...
  resize(e, t) {
    this.canvas.width = e, this.canvas.height = t, this.canvas.style.width = `${e * this.scale}px`, this.canvas.style.height = `${t * this.scale}px`, this.ctx.imageSmoothingEnabled = SMOOTH_RENDERING, SMOOTH_RENDERING && (this.ctx.imageSmoothingQuality = "high"); // nightshift: issue #23
  }
  getScale() {
    return this.scale;
  }
...
class oe {
  ...
  // --- Rendering ---
  renderBelow(e) {
    e.imageSmoothingEnabled = SMOOTH_RENDERING, SMOOTH_RENDERING && (e.imageSmoothingQuality = "high"); // nightshift: issue #23
...
const E = class E {
  ...
  autoSpawnCitizen(e) {
    // nightshift: role-based sprite mapping (issue #23) — derive sprite from agent ID (ns-team-role)
    const t = this.config.defaultSprites ?? ["nova", "rio", "dexter", "morty"];
    let i;
    if (e.id) {
      const _parts = e.id.split("-"), _role = _parts.slice(2).join("-").replace(/-\d+$/, "");
      i = ["producer", "planner", "reviewer", "coder", "tester"].includes(_role) ? _role : t[this.autoSpawnIndex % t.length];
    } else {
      i = t[this.autoSpawnIndex % t.length];
    }
    this.autoSpawnIndex++;
