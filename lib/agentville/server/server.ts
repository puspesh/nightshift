  ...
  private async handleWorldAction(agentId: string, action: { type: string; [key: string]: unknown }) {
    ...
    try {
      ...
      if (actionType === 'generate_texture') {
        const { prompt, id: rawTexId } = action as unknown as { prompt: string; id: string };
        if (!prompt || !rawTexId) { this.queueInbox(agentId, 'system', JSON.stringify({ error: 'generate_texture requires prompt, id' })); return; }
        const id = rawTexId.replace(/[^a-zA-Z0-9_-]/g, '');
      ...
      if (actionType === 'generate_prop') {
        const { prompt, id: rawPropId, w, h } = action as unknown as { prompt: string; id: string; w?: number; h?: number };
        if (!prompt || !rawPropId) { this.queueInbox(agentId, 'system', JSON.stringify({ error: 'generate_prop requires prompt, id' })); return; }
        const id = rawPropId.replace(/[^a-zA-Z0-9_-]/g, '');
  ...
  private async handleHttp(req: IncomingMessage, res: ServerResponse) {
    ...
    if (req.method === 'GET' && url.pathname === '/miniverse-core.js') {
      ...
      if (existsSync(corePath)) {
        ...
        res.end(JSON.stringify({ error: 'Core bundle not found' }));
      }
      return;
    }
    ...
    // Serve static world assets (tiles, props, sprites)
