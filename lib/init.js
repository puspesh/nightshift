...
export async function init(args) {
  ...
  console.log(`     ${chalk.cyan(`git add .claude/nightshift/ CLAUDE.md && git commit -m "chore: set up nightshift ${team} team"`)}`);
  console.log('');
  console.log('  3. Start the agents:');
  console.log(`     ${chalk.cyan(`/loop 15m @ns-${team}-producer`)}`);
  console.log(`     ${chalk.cyan(`/loop 15m @ns-${team}-planner`)}`);
  console.log(`     ${chalk.cyan(`/loop 15m @ns-${team}-reviewer`)}`);
  for (let i = 1; i <= coderCount; i++) {
    console.log(`     ${chalk.cyan(`/loop 15m @ns-${team}-coder-${i}`)}`);
  }
  console.log(`     ${chalk.cyan(`/loop 15m @ns-${team}-tester`)}`);
  console.log('');
}
